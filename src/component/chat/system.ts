...
const getSystemInstruction = async (): Promise<string> => {
    return `
        ...
        **E. FINAL ACTION: TOOL CALL (STRICTLY GUARDED)**
           **ONLY** upon receiving the explicit, standalone response of **"Yes"** from the user, call the **'bookAppointment'** tool.
        ...
        * Even after gathering all information from User, until User explicitly confirms with "Yes" you are not allowed to make the booking and call **'bookAppointment'** tool.
    `;
};
